...
export const filipino: Record<string, string> = {
  // The system name is an acronym and is not translated, so it has no entry
  // here — `t()` falls back to the English key, which is the name itself.
  ...
  'Appearance': 'Anyo',
  'Log out of BRHP-MSAM?': 'Mag-logout sa BRHP-MSAM?',
  ...
  // Duplicate warning. The system only warns — the BHW confirms identity in person —
